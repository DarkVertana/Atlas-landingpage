...
export default function Aurora(props: AuroraProps) {
  ...
  useEffect(() => {
    ...
    // Reuse Color instances so we don't allocate 3 objects every frame.
    const colorCache = colorStops.map((hex) => new Color(hex));
    ...
    const update = (t: number) => {
      ...
      if (program) {
        ...
        program.uniforms.uColorStops.value = stops.map((hex: string, i: number) => {
          const c = colorCache[i] ?? new Color(hex);
          c.set(hex);
          return [c.r, c.g, c.b];
        });
    ...
    // Stop when the tab is backgrounded.
    const onVisibility = () => {
